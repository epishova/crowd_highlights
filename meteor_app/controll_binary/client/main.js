...
Tracker.autorun(function() {
    ...
    if (group != null) {
        ...
        //Meteor.subscribe('getMainText', true, true);
        //Meteor.subscribe('getMarked', true, true);
        //Meteor.subscribe('getUHexperiments', true);
        //Meteor.subscribe('getTestText', true, true);
        //Meteor.subscribe('getTestMarked', true, true);
...
/*function SelectText(e) {
    console.log('focused', SelectText);
}*/
...
function checkQuiz(e) {
    ...
    if (exp && exp.quizDone) {
        ...
        return true;
            ...
            ((exp.experimentType != "3A" && document.getElementById('questEvents3201').checked ||
            exp.experimentType == "3A" && document.getElementById('questEvents3202').checked) ? 1 : 0) +
            ...
            ((exp.experimentType != "3A" && document.getElementById('questFamous2201').checked ||
            exp.experimentType == "3A" && document.getElementById('questFamous2202').checked) ? 1 : 0) +
...
Template.yesNo.helpers({
    ...
    'isDisabled': function(btn) {
        if (Session.get('timeoutSet'))
            ...
            //1-3}, 240000);
/*
                var n = Session.get('answerN');
                console.log("-------------answerN=", n);
                if (n) {
                    console.log("-------------1. set answerN to ", n + 1);
                    Session.set('answerN', n + 1);
                }
                else {
                    console.log("-------------2. set answerN to ", 1);
                    Session.set('answerN', 1);
                }
*/
            }, 0); //160000
            //-----1-3
...
*/
            ...
            if (exp && exp.resort == exp.resort1) {
                ...
                Meteor.call('sendAnswerN', 'Yes', 3, dateTime.msec);
                Meteor.call('goToExitSurvey');
            }
//----1-3
        }
...
Template.dbdata.helpers({
    ...
    'showExp3AB': function () {
        //console.log('mainText.find.count: ', mainText.find().count());
        var main = exp3AB.find().fetch();
        for (var j = 0; j < main.length; j++) {
            var expA = UHexperiments.findOne({groupId: main[j].idA});
            var expB = UHexperiments.findOne({groupId: main[j].idB});
            main[j].workerA = expA.workerId;
            main[j].workerB = expB.workerId;
        }
        return main;
    }
});
