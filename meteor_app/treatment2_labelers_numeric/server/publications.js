...
return UHexperiments.find({workerId: {$ne: "A3HYF5Y3FKIB3J"}, answer3MsecCl: {$gt: new Date(now - 1 * 24 * 60 * 60 * 1000)}});
...
var now = new Date().getTime();
return bigLog.find({currMsec: {$gt: new Date(now - 1 * 24 * 60 * 60 * 1000)}});
//return bigLog.find();
//    if (typeof(groupId) === "boolean" && groupId == true)
